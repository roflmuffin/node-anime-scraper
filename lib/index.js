...
var WEBSITE_ROOT = 'https://kissanime.to'
...
Anime.fromName = function (name) {
  ...
  AnimeUtils.searchByName(name).then(function (results) {
    var anime = _.findWhere(results, {name: name})
...
Anime.fromUrl = function (pageUrl) {
  ...
  got(pageUrl, OPTIONS, function (err, data, resp) {
    ...
    // A lot of items are not easily identifiable on the page
    // So we need to use a lot of (unreadable) jQuery to parse it all.
    info.names = $("span:contains('Other name:')").parent().text()
      .replace('Other name:', '').trim().split(';').map(function(s) { return s.trim() })
    info.genres = $("span:contains('Genres')").parent().text()
      .replace('Genres:', '').trim().split(',').map(function(s) { return s.trim() })
    info.summary = $("p:contains('Summary')").next().text().trim()
    info.airdates = $("p:contains('Date aired:')").text()
      .replace('Date aired:', '').trim().split(' to ')

    // Anime Name
    var animeName = $(".bigChar").text().trim()
